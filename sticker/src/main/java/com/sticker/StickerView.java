...
import com.ffmpeglashcard.linh.stickerview.R;
...
public class StickerView extends FrameLayout {
    ...
    private final int LIMITED_COUNT_STICKER = 20;
    ...
    protected boolean onTouchDown(@NonNull MotionEvent event) {
        ...
        if (handlingSticker != null) handlingSticker.setLocked(true);
        downX = event.getX();
        downY = event.getY();
    ...
    public Bitmap getBitmapResult() {
        if (handlingSticker != null) {
            handlingSticker.setLocked(true);
            invalidate();
        }
        return createBitmap();
    }
    ...
    public void saveSticker() {
        if (handlingSticker != null) handlingSticker.setLocked(locked);
        invalidate();
    }
    ...
    public boolean isLimited() {
        return stickers.size() > LIMITED_COUNT_STICKER ? true : false;
    }
